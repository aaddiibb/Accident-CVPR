...
8. [Example Input and Output](#8-example-input-and-output)
...
A dashcam video clip, 15 seconds long, 30 FPS, 1280×720:

```
Frame 0    → Normal traffic, 4 vehicles detected (IDs: 1, 2, 3, 4)
Frame 30   → Vehicles 2 and 3 converging (score: 0.12)
Frame 90   → Vehicles 2 and 3 very close (score: 0.41) ← candidate
Frame 105  → Vehicles 2 and 3 bounding boxes overlap (score: 0.87) ← TOP candidate
Frame 120  → Vehicles separated/stopped (score: 0.05)
```

### Intermediate Results

```
Interaction scores top candidates:
  Frame 105 (t=3.5s):  score=0.87,  pair=(2, 3)
  Frame  90 (t=3.0s):  score=0.41,  pair=(2, 3)
  Frame  60 (t=2.0s):  score=0.22,  pair=(1, 3)

VideoMAE features extracted: 14 clips around candidates
TriDet confidence scores:    [0.03, 0.07, 0.12, 0.71, 0.89, 0.82, 0.44, 0.09, ...]

Predicted accident_time:  3.48s
Colliding pair:           Track IDs (2, 3)
Impact bounding boxes:    bb1=[540, 310, 720, 430],  bb2=[680, 295, 860, 435]
Impact point:             px=710, py=367  →  cx=0.5547, cy=0.5097

Velocity of Track 2 (pre-collision): vx=-45.2, vy=+3.1  ← moving left
Velocity of Track 3 (pre-collision): vx=+38.7, vy=-2.4  ← moving right
Relative angle: 172°  → head-on
```

### Final Output Row

```csv
videos/clip_001.mp4,3.480,0.5547,0.5097,head-on
```

**Plain English interpretation:**
> *"At 3.48 seconds into the video, two vehicles collided head-on near the centre of the frame (slightly right of centre, roughly at the vertical midpoint). The vehicles were travelling in opposite directions."*

---

## 9. Performance & Limitations

### Speed Profile (approximate, GPU T4)

| Stage | Time per video |
|---|---|
| Frame loading (20s video) | ~2s |
| GroundingDINO detection (2 FPS) | ~25s |
| ByteTrack tracking | <1s |
| Interaction scoring | <1s |
| VideoMAE feature extraction (~18 clips) | ~30s |
| TriDet inference | <1s |
| Spatial + classification | <1s |
| **Total per video** | **~60s on GPU** |

> ⚠️ On CPU, GroundingDINO alone takes ~5min per video. **A GPU is strongly recommended.**

### Key Improvements in This Pipeline vs. Naive Approach

- **Candidate-only feature extraction** reduces VideoMAE calls from ~600 to ~18 per video (**33× speedup**)
- **ROI cropping** improves feature quality by removing background clutter
- **2 FPS detection** instead of 30 FPS provides **15× speedup** in the detection stage

### Known Limitations

| Limitation | Description |
|---|---|
| **Occlusion** | If vehicles are hidden behind trucks or buildings before collision, trajectories are incomplete and velocity estimates will be wrong |
| **Night / low light** | GroundingDINO's detection confidence drops significantly in dark conditions |
| **Fast collisions** | At 2 FPS detection, a very fast impact (<0.5s) might happen entirely between two sampled frames and be missed |
| **Single-vehicle accidents** | Collisions with walls, guardrails, or pedestrians are hard to detect since only vehicles are tracked |
| **Untrained TriDet** | If no CARLA synthetic data is found (`labels.csv` missing), TriDet falls back to the raw interaction score peak — less precise |
| **Rickshaw/unusual vehicles** | Despite GroundingDINO's flexibility, heavily occluded or unusual vehicles may still be missed |
| **Max 20 seconds** | Videos longer than 20 seconds are truncated — the accident might be in the latter portion |
| **Camera motion** | Fast-panning dashcams create large apparent vehicle velocities unrelated to actual vehicle motion |
...
pip install transformers accelerate safetensors timm einops
pip install supervision==0.22.0
pip install scikit-learn
pip install torch torchvision  # ensure CUDA version matches your GPU
```

### Dataset Structure

```
/kaggle/input/competitions/accident/
├── videos/                    ← test videos (.mp4)
└── sim_dataset/
    ├── labels.csv             ← training labels (accident_frame, rgb_path, ...)
    └── videos/                ← CARLA synthetic training videos
```

### Running on Kaggle

Simply run all cells in the notebook. The output will be saved to:

```
/kaggle/working/submission.csv
```

### Running on Google Colab (Single Video)

See the companion script `accident_detection_colab.py` for a version that:
- Takes a **single video** as input (upload via `files.upload()`)
- Produces an **annotated output video** with:
  - Bounding boxes per tracked vehicle
  - HUD overlay with accident time, coordinates, and type
  - Red impact-point crosshair at the moment of collision
  - "⚠ ACCIDENT DETECTED" banner

```python
# In Colab — just set this path after uploading
VIDEO_PATH = "your_video.mp4"
# Then run all cells → download accident_output.mp4
```
...
```
.
├── 2ndtry(2).ipynb              ← Main Kaggle notebook
├── accident_detection_colab.py  ← Single-video Colab script
└── README.md                    ← This file
```
